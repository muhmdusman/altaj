...
interface MenuCategoryListingPageProps {
  titlePrefix: string;
  titleAccent: string;
  description: string;
  categoryOrder: string[];
  includeRemainingCategories?: boolean;
...
const MenuCategoryListingPage: React.FC<MenuCategoryListingPageProps> = ({
  titlePrefix,
  titleAccent,
  description,
  categoryOrder,
  includeRemainingCategories = false,
  showDownloadButton = true,
}) => {
  const groupedItems = groupItemsByCategory(menuData as MenuItem[]);

  const baseCategories = categoryOrder.filter((category) => groupedItems[category]);
  const remainingCategories = includeRemainingCategories
    ? Object.keys(groupedItems).filter((category) => !categoryOrder.includes(category))
    : [];
  const orderedCategories = [...baseCategories, ...remainingCategories];
    ...
    <div className="min-h-screen bg-[#040402] text-white">
          ...
          <div className="flex flex-row items-center justify-between gap-4 mb-6">
            <div className="text-left">
              <h1 className="text-3xl sm:text-4xl md:text-5xl lg:text-6xl font-bold text-white">
                {titlePrefix} <span className="text-[#D4A541]">{titleAccent}</span>
              </h1>
            </div>
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-12">
        {orderedCategories.map((category) => {
          ...
          const formattedLabel = formatCategoryLabel(category);
            ...
            >
              <div className="text-center mb-8 lg:mb-12">
                <h2 className="text-3xl sm:text-4xl lg:text-5xl font-bold text-white mb-4">
                  <span className="text-[#D4A541]">{formattedLabel.charAt(0)}</span>
...
export default MenuCategoryListingPage;
